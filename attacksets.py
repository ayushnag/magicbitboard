# Credit to nkarve for tables.cpp
# Adapted to Python by Ayush Nag
# tables.cpp by github.com/nkarve under MIT License
...
def sliding_attacks(square: Square, occ: int, mask: int) -> int:
	return (((mask & occ) - SQUARE_BB[square] * 2) ^ reverse(reverse(mask & occ) - reverse(SQUARE_BB[square]) * 2)) & mask
...
def get_rook_attacks_for_init(square: Square, occ: int) -> int:
	return sliding_attacks(square, occ, MASK_FILE[file_of(square)]) | sliding_attacks(square, occ, MASK_RANK[rank_of(square)])
...
def initialise_rook_attacks(): 
	for sq in Square:
		edges: int = ((MASK_RANK[AFILE] | MASK_RANK[HFILE]) & ~MASK_RANK[rank_of(sq)]) | ((MASK_FILE[AFILE] | MASK_FILE[HFILE]) & ~MASK_FILE[file_of(sq)])
...
def get_xray_rook_attacks(square: Square, occ: int, blockers: int) -> int:
	attacks: int = get_rook_attacks(square, occ)
	blockers &= attacks
...
# the magic lookup table
def get_bishop_attacks_for_init(square: Square, occ: int) -> int:
...
def get_bishop_attacks(square: Square, occ: int) -> int:
	return BISHOP_ATTACKS[square][((occ & BISHOP_ATTACK_MASKS[square]) * BISHOP_MAGICS[square]) >> BISHOP_ATTACK_SHIFTS[square]]

# Returns the 'x-ray attacks' for a bishop at a given square. X-ray attacks cover squares that are not immediately
# accessible by the rook, but become available when the immediate blockers are removed from the board 
def get_xray_bishop_attacks(square: Square, occ: int, blockers: int) -> int:
	attacks: int = get_bishop_attacks(square, occ)
	blockers &= attacks
...
def initialise_squares_between():
	for sq1 in Square:
		for sq2 in Square:
			sqs: int = SQUARE_BB[sq1] | SQUARE_BB[sq2]
			if file_of(sq1) == file_of(sq2) or rank_of(sq1) == rank_of(sq2):
